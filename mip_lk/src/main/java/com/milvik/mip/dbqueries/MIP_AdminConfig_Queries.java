...
public class MIP_AdminConfig_Queries {
	...
	public static int getMaxLoginAttempt() {
		...
		try {
			...
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT value FROM admin_config where name='max_login_attempts';");
			result.next();
			maxCount = result.getInt("value");
	...
	public static Map<String, String> getAdminConfigInfo() {
		try {
			...
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT value FROM admin_config where name='default_password';");
			result.next();
			adminConfigdetails.put("default_password",
					result.getString("value"));
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT value FROM admin_config where name='user_login_prefix';");
			result.next();
			adminConfigdetails.put("user_login_prefix",
					result.getString("value"));
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT value FROM admin_config where name='password_history_limit';");
			result.next();
			...
			adminConfigdetails.put("max_login_attempts",
					result.getString("value"));
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT value FROM admin_config where name='max_idle_count';");
			result.next();
			adminConfigdetails.put("max_idle_count",
					result.getString("value"));
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT value FROM admin_config where name='announcement_message';");
			result.next();

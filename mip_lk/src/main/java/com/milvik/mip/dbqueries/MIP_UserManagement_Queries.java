	...
	public static Map<String, String> getUserInfo(String mobilenum) {
		try {
			...
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT user_uid,fname,sname,dob,gender,email_id,role_name,branch_name,is_supervisor FROM user_details ud join role_master rm on ud.role_id=rm.role_id join branch_details bd on ud.branch_id=bd.branch_id where ud.msisdn='"
							+ mobilenum + "';");
			while (result.next()) {
			...
			if (userdetails.get("is_supervisor").equalsIgnoreCase("no")) {
				result = MIP_DataBaseConnection.st
						.executeQuery("select concat(fname,' ',sname) as name from mip_lk_dg.user_details where user_id=(select supervisor_id from mip_lk_dg.user_details ud where is_supervisor=0 and ud.msisdn="
								+ mobilenum + ");");
